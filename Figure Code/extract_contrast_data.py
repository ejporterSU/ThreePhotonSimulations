...
save = True
fname = "3v_seq_phase_contrast_040726"
...
for RID in RIDS:
    ...
    # axs[0].imshow(1-ims[20],cmap=plt.get_cmap('Blues')) 
    # axs[1].imshow(1-ims[-20],cmap=plt.get_cmap('Blues')) 
    ...
    counts, hbins, _ = ax2.hist(xc.flatten(), bins=20, alpha=0.5, color='blue', orientation='horizontal', edgecolor='black')
    # ax2.set_ylim(-0.05,1.05)
    hbins = np.array(hbins)
    hbins = (hbins[1:] + hbins[:-1])/2
    p_guess = [np.mean(counts[5:-5]), np.mean(hbins), max(xc_avg) - min(xc_avg)]

    popt1,pcov1 = curve_fit(batman, hbins, counts, p0=p_guess, maxfev=100000)
    ax2.plot(batman(np.linspace(min(hbins),max(hbins),100),*popt1),np.linspace(min(hbins),max(hbins),100), color='red' )
    ...
    if save:
        data = {
            ...
            "hist_bins": hbins.tolist(), "hist_counts": counts.tolist(),
            "sine_popt": popt.tolist(),
            "batman_popt": popt1.tolist(),
            "p2p": float(max(xc.flatten()) - min(xc.flatten()))
        }
        with open(f"{_DATA_DIREC}/{fname}_{round(T_ramsey*1e6, 1)}us.json", "w") as f:
            json.dump(data, f, indent=2)
        print(f"Saved to {_DATA_DIREC}/{fname}.json")
    else:
        print(f"{_DATA_DIREC}/{fname}_{round(T_ramsey*1e6, 1)}us.json")
