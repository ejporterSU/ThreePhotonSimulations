...
\begin{scope}[shift={(0,0)}]
  ...
  \node[black, font=\Large] at (8, -1.6) {$\Phi  = \boxed{\vec{k}\cdot(\vec{x}_0 + \vec{v}t_a) + \phi_a - \omega_e(t_b-t_a)} \boxed{-\vec{k}\cdot(\vec{x}_0+\vec{v} t_b) + \phi_b}$};
...
\begin{scope}[rotate=3,shift={(1, 1)}]
  ...
  \node[font=\large, rotate=5] at (1.0 + \boxwidthb + \boxgap+ \boxwidtha -0.3,1.5 +1) {$\vec{k}_R = -\vec{k}_1$};
  ...
  \node[font=\large, rotate=5] at (1.0 + \boxwidthb*1.5 + \boxgap+ \boxwidtha ,1.5 + 1) {$|{}^1\text{S}_0\rangle + e^{i\Phi}|{}^3\text{P}_0\rangle$} ;
