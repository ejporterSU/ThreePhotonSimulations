...
def make_figure():
    ...
    # Plot gridspec
    gs = GridSpec(2,1, hspace=0.35, wspace=0.32, left=.05, right=0.98, top=0.94, bottom=0.18)
    #ax_p1 = fig.add_subplot(gs[0])
    ax_p2 = fig.add_subplot(gs[0])
    ...
    add_panel_label(ax_p2, 'a)', x=-0.15,y=1.05)
    add_panel_label(ax_p3, 'b)', x=-0.15, y=1.05)
    ...
    ax_p3.set_xlim([-0.02, 0.38])
    ax_p3.tick_params(axis='both', direction='in', which='both', width=TICK_WIDTH)


    ax_p3.scatter(t_raw_3, pop_1s0_3,
                  s=75, marker='o', ec='k', label=r'$^1S_0$',color=COLOR_1S0, zorder=6)
    ax_p3.scatter(t_raw_3, pop_3p1_3,
                  s=75, marker='^', ec='k', label=r'$^3P_1$',color=COLOR_3P1, zorder=6)
    ax_p3.scatter(t_raw_3, pop_3p0_3,
                  s=75, marker='s', ec='k',label=r'$^3P_0$', color=COLOR_3P0, zorder=6)

    ax_p3.set_xlabel(r'Sequence Duration (μs)', fontsize=FS_LABEL)
    ax_p3.legend(loc='upper right', fontsize=14, frameon=False, ncol=3,labelspacing=0.8,
                    columnspacing=1, handlelength=0, handletextpad=0.5,bbox_to_anchor=(0.98,1.04))
