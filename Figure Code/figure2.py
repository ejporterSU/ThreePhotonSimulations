...
# ── Panel (b) data ─────────────────────────────────────────────────────────────
data_ro1 = np.loadtxt('Data/RabiFloppingData1.txt', delimiter=',', skiprows=2)
data_ro2 = np.loadtxt('Data/RabiFloppingData2.txt', delimiter=',', skiprows=2)
...
ax1.set_ylabel('Linewidth (kHz)', fontsize=FS_AXIS)
ax1.set_xlim(0.05, 3e3)
ax1.set_ylim(0.1, 4e4)
ax1.set_xscale('log')
ax1.set_yscale('log')
...
ax2.tick_params(axis='both', direction='in', which='both', width=1.5)
...
img = plt.imread('Data/cloud_image.png')
axins_img = ax2.inset_axes([0.02, 0.58, 0.15, 0.3])
axins_img.set_xticks([])
axins_img.set_yticks([])
axins_img.imshow(img)
axins_img.text(0.5, -0.2, 'Atomic\nCloud', ha='center', va='center',
               fontsize=9, color='#1d1c1c', weight='bold',
